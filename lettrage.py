...
st.title("Lettrage de Excel (versão segura)")

uploaded_file = st.file_uploader("Seleciona o ficheiro Excel", type=["xlsx"])
...
if uploaded_file:
    ...
    if st.button("🔄 Processar ficheiro"):
        try:
            # Ler o ficheiro diretamente em memória
            ...
            # --- 1. Pares positivos/negativos na coluna J ---
            valores_j = {}
            ...
            # --- 2. Últimos 8 caracteres da coluna L em E, L, M (linhas diferentes) ---
            for i in range(2, max_linhas + 1):
                celula_L = folha_ultimos8.cell(row=i, column=12).value
                if celula_L is None:
                    continue
            ...
            # Guardar o resultado em memória
            output = BytesIO()
            wb.save(output)
            output.seek(0)
